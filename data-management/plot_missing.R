  ...
  if (checkSS %in% c("GW", "COW")) {
    ...
    p <- ggplot(full_mat, aes_string(x = time, y = space, fill = ".ssmssng")) +
      ...
      scale_fill_manual("", drop = FALSE, 
        values = c("Complete in ss" = hcl(195, 100, 65), "Complete out ss" = hcl(15, 100, 20),
                   "Mssng. values in ss"  = hcl(15, 100, 65), "Mssng. values out ss"  = hcl(15, 100, 45),
                   "No obs. in ss"   = hcl(15, 100, 85), "No obs. out ss"   = hcl(15, 0, 97))) +
    ...
    p <- ggplot(full_mat, aes_string(x = time, y = space, fill = "z")) + 
      geom_tile() +
      scale_fill_manual("", drop = FALSE,
                        values = c("Complete" = hcl(195, l=65, c=100), 
                                       "Mssng. values"  = hcl(15, l=65, c=100), 
                                       "No obs."   = hcl(15, l=97, c=0))) +
      scale_x_date(expand=c(0, 0))
  }
  p
}
